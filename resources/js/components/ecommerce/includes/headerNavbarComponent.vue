                    ...
                    <nav class="navbar">
                        ...
                        <div class="collapse navbar-collapse" id="myNavbar" :loop="false">
                            <ul class="nav navbar-nav menu_list">
                                ...
                                <li :class="{'dropdown': true, active: isActive == 'category'}">
                                    <router-link to="/categories" class="dropdown-toggle">{{ $t('category') }}<span class="caret"></span></router-link>
                                    <ul class="dropdown-menu">
                                        <li :class="{'dropdown': true}" v-for="(menu, index) in menuItems" :key="index"><a class="dropdown-toggle" @click="searchCategoryCrypt(menu.id)"> {{ $i18n.locale == 'ar' ? menu.category_name_arabic : menu.category_name }}<span v-if="menu.childCategory.length > 0" class="caret"></span></a>
                                            <ul class="dropdown-menu sub_menu" v-if="menu.childCategory.length > 0">
                                                <li v-for="(childMenu, index) in menu.childCategory" :key="index"><a @click="searchCategory(childMenu.category_name)">{{ $i18n.locale == 'ar' ? childMenu.category_name_arabic : childMenu.category_name }}</a></li>
                                            </ul>
                                        </li>
                                    </ul>
                                </li>
    ...
    export default {
        ...
        methods: {
            ...mapMutations([
                ...
                searchCategoryCrypt(value) {
                    var a = btoa(parseInt(value))
                    this.searchCategory(a)
                },
