...
export default {
    ...
    created () {
        ...
        this.setNavBar() // set navbar
        this.getBanner() // get Banner
    },
    mounted () {
    ...
    methods: {
        ...
        getSlider () {
            ...
            .then(response => {
                this.SET_SLIDER(response.data.data)
            })
        },
        getBanner () {
            axios.get('/api/banners')
            .then(response => {
                this.SET_BANNERS(response.data.data)
            })
        },
        getProducts () {
            ...
            .then(response => {
                ...
                this.SET_BEST_SELLER_PRODUCT(response.data.bestSeller)
                this.recentViews()
            })
            .catch(errorResponse => {
        ...
        setNavBar () {
            $(window).scroll(function(){
                if ($(window).scrollTop() >= 1) {
                    $('.site-branding-area').addClass('fixed-header');
                }
                else {
                    $('.site-branding-area').removeClass('fixed-header');
                }
            });
        }
