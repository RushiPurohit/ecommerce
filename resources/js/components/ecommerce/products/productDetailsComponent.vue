            ...
            <div class="item_product_detail">
                ...
                <div class="product_discription">
                    ...
                    <div class="">
                            ...
                            <div v-if="productOtherData && productOtherData.length > 0">
                                <div class="" v-for="(attribute, index) in productOtherData" :key="index">
                                    <label>
                                        ...
                                        <input type="radio" name="price" v-if="attribute.child_attributes != null" @click="changePrice(attribute.id, attribute.price, attribute.sale_price)" :checked="index == 0"> {{ attribute.child_attributes != null ? attribute.child_attributes.replace(/,/g, ' - ') : '' }}
                                    </label>
                        ...
                        <!-- <div  class="pad_adjust_copy" v-for="(value, key) in withAttribute" :key="key">
                            <label> {{key}} </label> -->
                            <!-- <div v-for="(attr, index) in value" :key="index">
                                <button class="btn btn-primary" @click="changePrice(attr.id, attr.price, attr.sale_price)">{{ attr.attribute }}</button>
                            </div> -->
                            <!-- <select class="dropdown" @change="getValue">
                                <option v-for="(attr, index) in value" :key="index" :value="key + ', ' + attr.id">{{ attr.attribute }}</option>
                            </select>
                        </div> -->
                    </div>
                    <h4>{{ $t('quantity') }}</h4>
                    ...
                    <h3 class="seller_name" v-if="productDetail.get_seller.role_id != 1">{{$t('seller')}}: {{ getLocale == 'ar' ? productDetail.get_seller.first_name_arabic : productDetail.get_seller.first_name }} {{ getLocale == 'ar' ? productDetail.get_seller.last_name_arabic : productDetail.get_seller.last_name }} </h3>
                </div>
            </div>
            <div class="panel panel-default">
                <ul class="nav nav-tabs prod_detail">
                    <li class="active"><a data-toggle="tab" href="#description">{{ $t('description') }}</a></li>
                    <!-- <li><a data-toggle="tab" href="#review">{{ $t('review') }}</a></li> -->
                    <li><a data-toggle="tab" href="#specification">{{ $t('specification') }}</a></li>
                </ul>
                        ...
                        <div id="specification" class="tab-pane fade in">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped confirm text-left">
                                    <tbody>
                                        <tr v-for="(spec, index) in specification" :key="index">
                                            <td style="width:30%"> {{ $i18n.locale == 'ar' ? spec.key_arabic : spec.key }}</td>
                                            <td>{{ $i18n.locale == 'ar' ? spec.value_arabic : spec.value }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
...
export default {
    ...
    methods: {
        ...
        productDetails() {
            ...
            if (this.$route.params.id) {
                ...
                .then (response => {
                    ...
                    this.withAttribute = response.data.attributes
                    this.withNoAttribute = response.data.noAttribute
                    ...
                    // tamplate rendering error preventing
                    this.loaded = true
                })
            }  
        },
        ...
        getValue (e) {
            console.log(e.target.value, this.selectValue)
