            ...
            <div class="container">
              <div  class="cart_body"  v-if="cartList.length > 0" >
                  <transition-group  mode="out-in"
                    ...
                    :duration="500">
                    <div class="cart_item" v-for="(cart, index) in cartList" :key="cart.productId">
                            <div class="image_container">
                                <a @click="setImage(cart.productData.product_other_image[0], cart.productData.id)"><img  alt="poster_1_up" class="shop_thumbnail" :src="cart.productData.product_other_image[0]"></a> 
                            </div>
                            <div class="detail_container">
                                <h2><a @click="setImage(cart.productData.product_other_image[0], cart.productData.id)">
                                ...
                                <p v-html="getLocale == 'ar' ? cart.productData.product_description_arabic : cart.productData.product_description"></p>
                                 <h3 class="seller_name" v-if="cart.productData.get_seller.role_id != 1">{{$t('seller')}}: {{ getLocale == 'ar' ? cart.productData.get_seller.first_name_arabic : cart.productData.get_seller.first_name }} {{ getLocale == 'ar' ? cart.productData.get_seller.last_name_arabic : cart.productData.get_seller.last_name }} </h3>
                            </div>
                            <div class="prooduct_price"> 
                                ...
                                <div class="product_quantity buttons_added">
                                    <input type="button" class="minus quantity-left-minus btn btn-danger btn-number" value="-" @click="removeFromCart(cart.productId)">
                                    ...
                                    <input type="button" class="plus quantity-right-plus btn btn-success btn-number" value="+" @click="addToCart(cart.productId, cart.price)">
                                </div>
                            ...
                            <div class="product_Remove">
                                <h3 class="price_tag" ><a @click="removeCartItem(index, cart.productId)"><i class="fas fa-trash-alt"></i></a></h3>
                            </div>
                    </div>
                  </transition-group>
                ...
                <div v-else>
                    <div class="product-content-right">
                        <div class="else_cart_empty">
                            <img v-if="getLocale == 'ar'" src="/img/empty-cart-arabic.png" >
                            <img v-else src="/img/empty-cart.png" >
...
export default {
    ...
    methods: {
        ...
        changeCart (productId, price, index) {
            ...
            if(cartValue > 0) {
                // Call the homeComponent method addToCart
                this.addToCart(productId, price, parseInt(cartValue))
            } else {
                // Call the homeComponent method removeFromCart
