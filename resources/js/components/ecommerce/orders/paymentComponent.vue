                ...
                <div class="item_payment panel panel-default">
                    ...
                    <div class="panel-body" v-if="paymentMethod == 'card'">
                        ...
                        <!-- <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="full_name_id" name="full_name" :placeholder="$t('nameOnCard')" data-vv-name="nameOnCard" v-model="nameOnCard" v-validate="'required'"  :class="{'alert-danger': errors.has('nameOnCard')}" />
                        </div>	

                        <div class="form-group ">
                            <label for="full_name_id" class="control-label">{{ $t('cardNumber') }}</label>
                            <input type="text" class="form-control" id="cardNumber_id" name="cardNumber" :placeholder="$t('cardNumber')" data-vv-name="cardNumber" v-model="cardNumber" v-validate="'required'"  :class="{'alert-danger': errors.has('cardNumber')}" />
                        </div>
                   
                       <div class="fast_last">
                        <div class="form-group">
                            <label for="street1_id" class="control-label">{{ $t('expiryDate') }}</label>
                            <input type="text" class="form-control" id="street1_id" name="expiryDate" :placeholder="$t('expiryDate')" data-vv-name="expiryDate" v-model="expiryDate" v-validate="'required'"  :class="{'alert-danger': errors.has('expiryDate')}" />
                            
                        </div>					
                                                
                        <div class="form-group">
                            <label for="area_id" class="control-label">{{ $t('cvv') }}</label>
                            <input type="text" class="form-control" id="area_id" name="cvv"   :placeholder="$t('cvv')" data-vv-name="cvv" v-validate="'required'" v-model="cvv"  :class="{'alert-danger': errors.has('cvv')}"  />
                        </div>	 
                       </div> -->
                        <div class="pad_adjust">
                            <PayPal
                                :amount="grandTotal ? grandTotal.toString() : CART_TOTAL_PRICE.toString()"
                                currency="USD"
                                env="sandbox"
                                :client="credentials"
                            ...
                            <!-- <input type="submit"  class="btn btn-warning btn-block" :value="$t('checkOut')"/> -->
                            <!-- <input type="submit"  class="btn btn-warning btn-block"  @click="callParent" :value="$t('placeOrder')"/> -->
                        </div> 
                 </div>
                </div>            
...
export default {
    ...
    data () {
        return {
            ...
            cvv: '',
            credentials: {
                sandbox: 'Abugzwi4isPE-5Rj-sZcnGLHScBUO2BtJekQ5nmhD4xmPA5j6Fzl1sOwxQ1GsPjpgVSuyotX2u0a4aQI',
    ...
    methods: {
        ...
        callParent() {
            this.$parent.$parent.addPayment(this.paymentMethod)
            this.placeOrder()
        },
